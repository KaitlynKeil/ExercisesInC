...
I got an error around 500 threads, when the final value was 499. Whoops.
...
So many. In fact, we got 131150 errors.
...
So... synchronization didn't actually seem to take up any time. In fact, I got the following results:
...
I ran it a few times and consistently got this opposite behavior. I'm not sure why. Once (and only once), the mutex code took `0.004s` instead of `0.000s`, so maybe that's enough. It is essentially negligible.
