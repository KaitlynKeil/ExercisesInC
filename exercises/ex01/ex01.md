...
The two lines
        subq    $16, %rsp
        movl    $5, -4(%rbp)
seem to correspond to this line. This would probably be moving the current place in memory by 16 bits, then inserting the value of 5 4 bits back.

2. What happens if you turn on optimization using the flag `-O2`?

Many things change. Different print statement, different essentially everything. It does it in a different order that cuts out redundancy, possibly not even storing the x since that vaule is not ever used.
...
It calls a different print function (printf_chk), takes fewer movl actions, and uses a few different functions (xorl, addq) in order to use fewer lines overall.
...
It looks like several of the moves taken in the unoptimized version get collected into single functions when the optimized version runs. For example, instead of storing 5 for X, it just skips to assigning 6 to Y.
